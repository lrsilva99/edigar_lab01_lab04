...
import com.puc.pos.lab1.web.rest.util.HeaderUtil;
import com.puc.pos.lab1.web.rest.util.PaginationUtil;
...
public class CustomerResource {
    ...
    @RequestMapping(value = "/customers/{name}/{site}/{cnpj}/{address}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Customer>> filterFullColumn( @RequestParam(value="name", required = true) String name,
                                                               @RequestParam(value="nite", required = true) String site,
                                                               @RequestParam(value="cnpj", required = true) String cnpj,
                                                               @RequestParam(value="address", required = true) String address,
                                                               Pageable pageable)
        throws URISyntaxException {
        log.debug("REST retornar um determinado usuário na pagina");
        Page<Customer> page = customerService.filterFullColumn(name,site,cnpj,address,pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/customers");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
    ...
    @RequestMapping(value = "/customers/{from}/{to}",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Customer>> findFromTo( @RequestParam(value="From", required = true) int from,
                                                               @RequestParam(value="To", required = true) int to,
                                                               Pageable pageable)
        throws URISyntaxException {
        log.debug("REST retornar a paginação de acordo com os critérios do usuário");
        List<Customer> listCustomer = customerService.findFromTo(from, to);
        return Optional.ofNullable(listCustomer)
            .map(result -> new ResponseEntity<>(
                result,
                HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
